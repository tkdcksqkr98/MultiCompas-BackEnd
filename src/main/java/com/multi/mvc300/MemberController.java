...
public class MemberController {
	 ...
	 * session) { System.out.println(bag); //dao를 이용해서 db처리할 예정. //view아래로 넘어가게
	 * 되어있음. //view아래 login.jsp를 호출하게 됨. int result = dao.login(bag); //1, 0 if
	 * (result == 1) { //로그인이 성공하면, 세션을 잡아두자!!! session.setAttribute("id",
	 * bag.getId()); return "ok"; //views아래 파일이름.jsp }else { //views아래가 아니고,
	 * webapp아래 //member.jsp로 가고 싶은 경우! return "redirect:member.jsp"; //로그인실패했을때
	 * 창이넘어가지않음 }
	 * 
	 * }
	 */
...
//	public void update2(MemberVO bag) {
//		System.out.println("update요청됨.");
//		System.out.println(bag);
//		//dao.update(bag);
//	}
